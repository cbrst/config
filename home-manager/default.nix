...
let
  ...
  firefoxUserChrome =
    lib.replaceStrings
      [ "@firefoxTheme@" ]
      [ "file://${config.xdg.stateHome}/config-theme/firefox-wayland.css" ]
      (builtins.readFile "${dotfiles}/firefox/userChrome-wayland.css");
  tridactylThemeLayout = builtins.readFile "${dotfiles}/tridactyl/themes/_global.css";
  mkTridactylTheme = palette:
    tridactylThemeLayout + "\n" + builtins.readFile "${dotfiles}/tridactyl/themes/${palette}.css";
...
in
{
  ...
  xdg = {
    ...
    configFile = {
      ...
      "opencode/skills".source = lib.mkDefault "${dotfiles}/opencode/skills";
       # Tridactyl does not support CSS imports, so deploy each palette with its shared layout.
       "tridactyl/tridactylrc".source = lib.mkDefault "${dotfiles}/tridactyl/tridactylrc";
       "tridactyl/editor.sh".source = lib.mkDefault "${dotfiles}/tridactyl/editor.sh";
       "tridactyl/themes/meowsoot.css".text = lib.mkDefault (mkTridactylTheme "meowsoot");
       "tridactyl/themes/monokai-pro.css".text = lib.mkDefault (mkTridactylTheme "monokai-pro");
  ...
  home.activation.initializeThemeState = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    # Keep browser runtime imports aligned with the selected family without mutating linked assets.
    ...
    install -Dm644 ${lib.escapeShellArg "${dotfiles}/firefox/themes"}/"$family".css \
      "$state_dir/firefox-wayland.css"
